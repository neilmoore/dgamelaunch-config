...
list_hashes()
{
	...
	if verbose; then
		...
		for folder in $(/bin/ls -1td crawl-svn-*)
		do
			/bin/ls -1tdl ${folder} 2>/dev/null | \
				sed 's/crawl-svn-//g;' | \
				awk '{ printf "%3s %2s  %s    %-9s  %6s %6s    ", $6, $7, $8, $9, "'$(echo select major,minor from versions where hash=\"${folder/crawl-svn-/}\"\; | sqlite3 -separator \. ${VERSIONS_DB})'", "'$(/bin/ls -1 ${folder}/saves/*.sav ${folder}/saves/*.chr ${folder}/saves/*.cs ${folder}/saves/sprint/*.cs ${folder}/saves/zotdef/*.cs 2>/dev/null | wc -l)'" }'
...
do
	if test -f bin/crawl-svn-${version} -a -d crawl-svn-${version}
	then
		if test ${WAIT_KEY} -eq 1
		then		
			while { ps -fC crawl-svn-${version} | awk '{ print $1" "$2"\t "$5" "$7"\t "$8" "$9" "$10 }' | grep ^dgl; }
			do
			...
			rm bin/crawl-svn-${version}
			rm -r crawl-svn-${version}
		else
			if { ps -fC crawl-svn-${version} | awk '{ print $1" "$2"\t "$5" "$7"\t "$8" "$9" "$10 }' | grep ^dgl >/dev/null; }
			then
