...
sub main() {
  ...
  if (not $ver) {
    do_prompt 'trunk', '0.13', '0.14', '0.15', 'nostalgia', 'chunkless', 'cards', 'salamander', 'no_backtracking_god', 'faithful';
  } elsif ($ver eq 'trunk') {
    do_update 'trunk';
  } elsif ($ver =~ /^0.1[345]|nostalgia|cards|salamander|faithful|no_backtracking_god|chunkless$/) {
