...
sub main() {
  ...
  if (not $ver) {
    do_prompt 'trunk', '0.13', '0.14', '0.15', '0.16',  'nostalgia', 'salamander', 'no_backtracking_god', 'evoker-god', 'shoals-lite', 'bearkin', 'faithful';
  } elsif ($ver eq 'trunk') {
    do_update 'trunk';
  } elsif ($ver =~ /^0.1[3456]|nostalgia|salamander|faithful|evoker-god|shoals-lite|bearkin|no_backtracking_god$/) {
