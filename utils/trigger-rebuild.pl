...
sub main() {
  ...
  if (not $ver) {
    do_prompt 'trunk', '0.13', '0.14', '0.15', '0.16',  'nostalgia', 'salamander', 'no_backtracking_god', 'evoker-god', 'imp', 'bearkin', 'mulch_ado_about_nothing', 'squarelos-0.17', 'faithful';
  } elsif ($ver eq 'trunk') {
    do_update 'trunk';
  } elsif ($ver =~ /^0.1[3456]|nostalgia|salamander|faithful|evoker-god|imp|bearkin|squarelos-0.17|mulch_ado_about_nothing|no_backtracking_god$/) {
